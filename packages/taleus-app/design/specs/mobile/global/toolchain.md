...
- Three of the libraries above are not yet installed, deliberately. Each is tracked, so a slice
  ...
  - **react-navigation** — blocked by the app's React Native version, not by the library: current
    react-native-screens needs RN 0.84+ codegen and the app is on 0.82.1. A local navigator with the
    same `{ route, navigation }` shape stands in. `debt-mobile-navigation-library`
