...
import { insertStatement, openStrand, row, rows, type RowWrite, type SchemaName } from './strand.js'
...
/** Sign a digest with a party's key, in the base64url form the schema stores. */
export function signText(key: KeyPairText, digestText: string): string {
	return Buffer.from(sign(key.secretKey, Buffer.from(digestText, 'utf8'))).toString('base64url')
}
