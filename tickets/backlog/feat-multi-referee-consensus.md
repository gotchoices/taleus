...
files: packages/taleus-core/schema/draft1.qsql (PendingLift, Ledger LiftFinalize, LiftVoid), packages/taleus-core/src/lift/referee.ts, docs/architecture.md (§ Referee model and the commit seam)
----
...
- **Agent/referee.** `packages/taleus-core/src/lift/referee.ts` runs multiple referee votes; the commit path collects a majority of per-edge signatures before an edge may settle.
