...
- **sApp schema.** The strand applies the Taleus schema ([`packages/taleus-core/schema/`](../packages/taleus-core/schema/)) alongside the standard Sereus `Strand` membership schema. All tally rules live in the schema as constraints — there is no trusted server to enforce them.
...
The schema ([`packages/taleus-core/schema/`](../packages/taleus-core/schema/)) follows a few uniform rules:
...
| `CloseRequest` | A signed **close request** — one row per party (either or both may file), moving the tally to *Closing*. Unilateral and safe: it freezes new credit in either direction (a right each party already holds) while balance-*reducing* direct chits and lift pledges are always accepted, so the tally winds down toward zero without ever trapping the counterparty's balance. Distinct from setting a zero credit limit, which leaves the tally *Open*. Insert-only, requester-signed, `Cid`-bound digest. |
| `Ledger` | Chits: issuer, units (positive integer, smallest denomination unit), date, reference/memo, issuer signature, chained balance, and an optional `InvoiceId` linking the chit to the invoice it answers. A `Kind` column distinguishes a **direct** chit (issuer-signed) from a finalized **lift** chit (`Kind = 'lift'`: issuer signature/`InvoiceId` absent, authorized instead by the referee's commit signature over the pending pledge). Both carry a real settled balance delta and chain identically. |
| `PendingLift` | A **conditional** lift pledge on this tally (edge), issuer-signed like a direct chit but keyed by the lift's globally-unique `LiftId` and naming the agreed **referee**. It **reserves** capacity while open but does *not* move the settled balance; it settles into `Ledger` (as a `Kind = 'lift'` row) only if the referee commits, or is voided by `LiftVoid`. This is how many independent tally strands commit a lift atomically with no cross-strand transaction — each strand independently verifies the *same* referee signature. |
| `LiftVoid` | The referee's signed **void** of a pending lift — an explicit abort or a post-timeout void (the same mechanism; parties never unilaterally void). One row per lift; its digest is distinct from the commit digest, so a commit signature can never be replayed as a void. Voiding writes no settled row — the pledge drops out of the reservation set, releasing its capacity. |

Alongside the tables, the schema defines views. `RegisteredKey` is every key ever introduced for a party — normal `PartyKey` adds plus counterparty-attested `PartyKeyAdoption` keys — and `AuthorizedKey` is that set minus every `PartyKeyRevocation`: the authoritative "who may sign for this party right now" set that all signature-gated tables resolve their `SignerKey` against. The rest compute lift capacity in place. `PerspectiveBalance` is the **settled** balance as each party sees it (direct chits plus *finalized* lifts — positive means accumulated value); it is the authoritative signed balance. A parallel **reserved** balance folds in *open* pending lifts: `OpenPendingLift` is the unresolved-pledge set (a `PendingLift` with neither a finalize nor a void), `ReservedBalance` adds those pledges' deltas to the settled balance, and `ReservedPerspectiveBalance` is that per party. New pledges and new direct chits are gated against the reserved balance so concurrent lifts cannot collectively over-commit, and `LiftLading` (per direction: units movable free up to the receiver's target, further units to its bound at the receiver's `Reward`, with the releasing party's `Clutch` applied to the whole amount) advertises capacity from the *reserved* balance, so an open pledge already shrinks advertised room. A finalize moves a pledge's delta from reserved into settled, leaving the reserved total unchanged. Two more derive **invoice** state, which is never stored: `InvoiceState` labels every invoice `open` / `declined` / `expired` / `paid` (precedence in that order, `paid` first, so the state stays deterministic even in a self-inflicted decline-and-pay race), and `OpenInvoice` is the still-open subset the lift agent reads as an advisory signal of upcoming balance movement — deliberately *not* folded into `LiftLading` or the hard credit gate. `CloseState` derives the tally's wind-down state — `open` (no close request filed), `closing` (a `CloseRequest` row exists but the tally is not yet a stable zero), or `closed` (settled `Ledger` balance zero *and* no open pending lift) — from `CloseRequest`, the settled balance, and `OpenPendingLift`. Trading variables live in the shared strand deliberately — they are signed, unilateral policy the counterparty's lift agent must read to advertise route capacity. Exchange rates, by contrast, span multiple tallies and stay in the party's private **portfolio** — a separate single-party strand ([Portfolio](#portfolio)), never this shared tally strand, so no counterparty ever reads them.
...
The portfolio lives in its own **single-party Sereus strand** — a closed strand whose only member is this party, carrying the Taleus portfolio schema ([`packages/taleus-core/schema/portfolio.qsql`](../packages/taleus-core/schema/portfolio.qsql)). A single-member closed strand is a supported Sereus configuration (the founder-bootstrap / solo path), and its cohort is just this party's own cadre, so ordinary strand replication carries the portfolio to every one of the party's nodes, including the always-on lift-agent node.
...
**Same `sAppId`, two schemas.** The phone runs one cadre node with one strand filter (`sAppId:taleus`), so both the portfolio strand and every tally strand share `sAppId = taleus` to be picked up. `sAppId` is a filter tag, not a schema identity — each `Strand` row names its own sApp schema, so one `taleus` sAppId hosts both the two-party tally schema ([`packages/taleus-core/schema/draft1.qsql`](../packages/taleus-core/schema/draft1.qsql)) and the single-party portfolio schema. The app tells a portfolio strand from a tally strand by the `PortfolioCore` marker row (below), not by member count — a tally mid-formation is also briefly one member.
...
The `ValidDenomination` check (see [`packages/taleus-core/schema/draft1.qsql`](../packages/taleus-core/schema/draft1.qsql)) enforces exactly these three shapes at proposal and at acceptance; malformed identifiers (`usd`, `iso4217:US`, `iso4217:usd`, an empty `cid:`) are rejected.
...
A party that holds tallies in more than one denomination stores its conversion policy as **exchange rate quotes** in its portfolio ([`packages/taleus-core/schema/portfolio.qsql`](../packages/taleus-core/schema/portfolio.qsql), `ExchangeRateQuote`). A quote is *private* — read only by the party's own lift agent at decision time, never shared into a tally strand. This is the key distinction from a **trading variable** (`TradingVariable`), which a party publishes *into the shared tally strand* for the counterparty to read: an exchange rate is the party's own internal cost of crossing between two of its denominations, so no counterparty ever sees it.
...
**Overflow.** `req_out × RateNum × 10^(s_in)` overflows 64 bits on a large amount × large rate × large scale, so the conversion helper reduces `RateNum/RateDen` to lowest terms and computes the intermediate product with **BigInt** (cross-platform), taking the ceiling via integer division. This is a fixed decision, recorded as a `NOTE:` at the rate definition in [`packages/taleus-core/schema/portfolio.qsql`](../packages/taleus-core/schema/portfolio.qsql) and implemented at [`packages/taleus-core/src/lift/convert.ts`](../packages/taleus-core/src/lift/convert.ts) (`convertBoundary`) — a pure function with no ChipNet or network dependency, so it does not regress to a native 64-bit multiply. The lift agent (`feat-chipnet-integration`) is its consumer, not its home.
...
- **Credit gate**: every direct chit is rejected unless the resulting balance stays within the credit each party has extended — the `WithinCreditLimits` constraint checks the prospective **settled** balance against each grantor's limit *effective as of the chit's own signed date* (the highest-revision `CreditTerms` row whose `EffectiveDate` has arrived; no `CreditTerms` row means zero credit, so the first nonzero chit needs a published revision). Because the check is keyed to the chit's date it is deterministic — and because that date is asserted and signed by the issuer rather than read from any clock, the gate inherits whatever honesty the date has. `DateMonotonic` bounds how far back a chit may reach; the rest is the receiving agent's clock policy and, failing that, dispute. See [timestamps.md](timestamps.md). A second gate, `WithinReservedCredit`, additionally holds the *reserved* balance (settled plus every open pending lift) within the same limits, so a direct chit cannot consume capacity an open lift pledge has reserved. A referee-committed **lift finalize** (`Kind = 'lift'`) is *exempt* from both gates: once the referee has committed, the finalize must always settle, or a per-strand credit check could reject a finalize the other strands accept — breaking atomicity. The pledge was already gated when it was made; a committed lift is binding even if it pushes the settled balance past the nominal limit.
...
- **Lift chit**: during a lift, a conditional pledge is written into each edge as a `PendingLift` row, bound to the lift's `LiftId` and the agreed referee and issuer-signed like a direct chit. It *reserves* capacity — the reserved balance, `LiftLading`, and both reserved credit gates see it — but does not touch the settled balance. The referee then resolves the whole route with one signature: a **commit** finalizes each edge as a fresh `Ledger` row (`Kind = 'lift'`), verified locally against the pending pledge's terms and referee key; a **void** (explicit abort or post-timeout, recorded in `LiftVoid`) releases the reservation with no settled row. Commit and void are mutually exclusive both ways, and their digests are distinct so neither signature can be replayed as the other. Because every strand verifies the *same* referee signature, the many edges commit or cancel atomically with no cross-strand transaction.

### Tally close

Either party may wind a tally down by signing a **close request** (`CloseRequest`) — one row, either side, is enough; it is deliberately *unilateral*. Its presence moves the tally to **Closing** (`CloseState = 'closing'`), which does exactly two things and cannot trap value:

- **Freezes balance growth.** No new direct chit from either side may move the settled balance *away* from zero (the `ClosingReducesBalance` gate), and no new lift pledge may grow the reserved balance's magnitude (`ClosingReducesReserved`) — that is, no new credit is extended in either direction. This takes nothing the counterparty is owed: it is the same restriction a party can already impose by setting its `CreditTerms.CreditLimit` to the current balance.
- **Always permits reduction.** Balance-*reducing* direct chits and lift pledges are accepted from either party throughout Closing, so whoever holds a positive balance can always be paid down or lift its value out elsewhere in the graph. A referee-committed lift **finalize** (`Kind = 'lift'`) is exempt from the closing gate for the same atomicity reason it is exempt from the credit gates — the pledge was checked when made, and a committed lift must always settle.

The tally reaches **Closed** (`CloseState = 'closed'`) only when the **settled** balance is genuinely zero *and* no pending lift is still in flight — so a unilateral close never strands a counterparty's positive balance; it stays fully recorded and collectible while the tally sits in Closing. Requiring a real zero (not merely the presence of a close request) is what makes Closed a stable terminal state a late finalize cannot bump back off zero.

**Zero credit is not close.** Setting `CreditTerms.CreditLimit = 0` leaves the tally **Open**: no `CloseRequest` row exists, the balance may still be paid down, lifts still run, and credit can be restored by a later `CreditTerms` revision. Close is a distinct, separately-signed state, never a terms revision (see [`docs/old/tally.md`](old/tally.md) *§ Important Distinction: Zero Credit vs. Close Request*).
...
**`LiftId` is per-edge, not per-route.** Each edge's finalize digest binds *its own* tally `Cid`, so the referee's signature differs on every edge; the `{ LiftId → refereeEdgeSignature }` map can only tell those signatures apart if each edge carries its own `LiftId` (the strand's `PendingLift.LiftId` PK). The whole lift is correlated instead by the ChipNet `transactionCode` and the originator's `LiftJournal` row; the per-edge pledge ids live only on the strands and in the record `payload`.
...
- Storage on device via Quereus's NativeScript plugin (`quereus-plugin-nativescript-sqlite`); the `taleus-core` library itself is platform-neutral and also runs in Node (the `taleus-node` headless agent) and the browser.
...
| Closing | Close requested (a `CloseRequest` row exists, `CloseState = 'closing'`); new credit frozen, only balance-reducing direct chits and lift pledges accepted |
| Closed | Settled balance zero after close with no pending lift in flight (`CloseState = 'closed'`); strand may hibernate permanently / be archived |
| Void | Formation abandoned before contract signature |

**Closing** and **Closed** are schema-backed: the `CloseRequest` table, the derived `CloseState` view, and the two closing gates (`Ledger.ClosingReducesBalance`, `PendingLift.ClosingReducesReserved`) enforce them. **Forming**, **Open**, and **Void** depend on contract/formation state the schema does not materialize today, so `CloseState`'s `'open'` means only "no close request filed", not a distinction among those three. Once `CloseState = 'closed'`, the strand may be **permanently hibernated / archived via Sereus**; the insert-only, replicated data remains as dispute evidence. That archival is a Sereus/app-layer action, not a schema constraint.
